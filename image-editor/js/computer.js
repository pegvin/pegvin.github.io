...
var brushColorpicker = tui.colorPicker.create({
	container: $('#tui-brush-color-picker')[0], color: '#000000',
});

// Color picker for text palette
var textColorpicker = tui.colorPicker.create({
	container: $('#tui-text-color-picker')[0], color: '#000000',
});

// Color picker for shape
var shapeColorpicker = tui.colorPicker.create({
	container: $('#tui-shape-color-picker')[0], color: '#000000',
});

// Color picker for icon
var iconColorpicker = tui.colorPicker.create({
	container: $('#tui-icon-color-picker')[0], color: '#000000',
});

var tintColorpicker = tui.colorPicker.create({
	container: $('#tui-tint-color-picker')[0], color: '#000000',
});

var multiplyColorpicker = tui.colorPicker.create({
	container: $('#tui-multiply-color-picker')[0], color: '#000000',
});

var blendColorpicker = tui.colorPicker.create({
	container: $('#tui-blend-color-picker')[0], color: '#00FF00',
});

// Common global functions

function hexToRGBa(hex, alpha) { // HEX to RGBA
...
function base64ToBlob(data) {
	...
	raw = data.replace(rImageType, function (header, imageType) { mimeString = imageType; return ''; });
	raw = atob(raw);
...
function getBrushSettings() {
	...
	return { width: brushWidth, color: hexToRGBa(brushColor, 0.5), };
}
...
function activateIconMode() { imageEditor.stopDrawingMode(); }
...
function setShapeToolbar(obj) {
	...
	if (colorType === 'stroke') {
		if (!isTransparent && !isFilter) { shapeColorpicker.setColor(changeValue); } }
...
$btnImageFilter.on('click', function () {
	...
	tui.util.forEach(filters, function ($value, key) { $value.prop('checked', imageEditor.hasFilter(key)); });
	$displayingSubMenu.hide();
...
$btnLoadMaskImage.on('change', function () {
	...
	if (!supportingFileAPI) { alert('This browser does not support file-api'); }
...
$btnApplyMask.on('click', function () {
	imageEditor .applyFilter('mask', { maskObjId: activeObjectId, }).then(function (result) { console.log(result); });
});
